...
from src.demo_core import ALLOWED_MODELS, make_2d_case, run_root_name, train_pines_model
...
def main(
    ...
    case_data = make_2d_case(case_cfg)
    anchors = int(pines_cfg.get("equivalent_sources_2d", {}).get("anchors", 80))
    output_root = _resolve(Path(case_cfg.get("output_root", "output/2d")))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the standalone 2D PINES sound field demo.")
    parser.add_argument("--config", type=Path, default=Path("config/config_2d.yaml"))
